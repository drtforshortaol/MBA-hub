...
function buildSections() {
  ...
  const naturalHtml = `${imageHtml(["rookery_scene", "calendar", "diet_diagram"], "three")}${gridFromObject({Habitat: natural.habitat, Range: natural.range, Depth: natural.depth, Diet: natural.diet, Predators: natural.predators, Behavior: natural.behavior, Lifespan: natural.lifespan, "Conservation Status": natural.conservationStatus})}<div class="callout"><b>Adaptations:</b> ${escapeHTML(natural.adaptations.join(" "))}</div>`;
  ...
  return [
    ...
    { icon: "🔎", title: "Natural History", sub: "Habitat, behavior, diet, adaptations, and ecology", html: naturalHtml, searchText: `natural history habitat range depth diet predators behavior adaptations lifespan conservation rookery calendar diet ${Object.values(natural).join(" ")}` },
